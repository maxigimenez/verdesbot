...
VerdesBot.prototype._response = function (msg, cb) {
  ...
  if (commands.isHelp(msg.text)) {
    ...
    dolarblue(_.bind(function (err, data) {
      ...
      this.db.get('SELECT COUNT(*) as count FROM history WHERE date = "'+today+'"', _.bind(function (err, record) {
        if (record.count === 0) {
          this.db.run('INSERT INTO history(date, ambito_buy, ambito_sell, oficial_buy, oficial_sell) VALUES(?, ?, ?, ?, ?)', today, parsed.ambito_financiero.value_buy, parsed.ambito_financiero.value_sell, parsed.oficial.value_buy, parsed.oficial.value_sell);    
        }
      }, this));
    ...
    this.db.all('SELECT * FROM history', function (err, records) {
