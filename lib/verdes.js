...
VerdesBot.prototype._onMessage = function (message) {
  if (this._isChatMessage(message) &&
      ...
      !this._isFromVerdesBot(message) &&
      this._isMentioningVerdes(message)
  ) {
...
VerdesBot.prototype._response = function (originalMessage) {
  var channel = this._getChannelById(originalMessage.channel);
  var message = '';

  if (commands.isHelp(originalMessage.text)) {
  ...
  } else if (commands.isToday(originalMessage.text)) {
  ...
  } else if (commands.isSell(originalMessage.text)) {
    dolarblue(function (err, data) {
      var amount = commands.extractSellNumber(originalMessage.text);
    ...
    message = 'No te estaria entendiendo culia.';
    this.postMessageToChannel(channel.name, message, {
      as_user: true
    });
  }
